...
系統管理者需要將每天上傳到 NAS 指定目錄的文字檔案轉換為 parquet 格式。批次服務會掃描 NAS 目錄中的檔案，透過檔案前綴名稱從資料庫查詢對應的主檔規格（包含編碼、格式類型、欄位定義等），然後根據規格進行解析與轉換。這些文字檔案可能使用不同的編碼格式（big5 或 utf-8），並且資料可能以分隔符號或固定長度方式儲存。

**為什麼是這個優先級**：這是核心功能，沒有檔案掃描、規格查詢與格式轉換，整個服務就無法運作。此功能提供最基本的價值 - 將原始資料轉換為可用的格式。

**獨立測試**：可以透過在資料庫中預先建立主檔規格資料，在 NAS 測試目錄中放置對應前綴的測試檔案（包含不同編碼與格式），執行轉換流程，驗證產出的 parquet 檔案內容是否正確，無需依賴後續的遮罩或轉換服務。

**驗收情境**：

1. **假設** NAS 目錄中有檔案 `customer20251206.txt`，資料庫中有前綴為 `customer` 的主檔規格（big5 編碼、`||` 分隔），**當** 批次服務掃描 NAS 並查詢到對應規格後讀取該檔案並進行轉換，**則** 成功產生對應的 parquet 檔案，且資料內容與原始檔案一致
2. **假設** NAS 目錄中有檔案 `transaction20251206.txt`，資料庫中有前綴為 `transaction` 的主檔規格（utf-8 編碼、`@!!@` 分隔），**當** 批次服務掃描 NAS 並查詢到對應規格後讀取該檔案並進行轉換，**則** 成功產生對應的 parquet 檔案，且資料內容正確解析
3. **假設** NAS 目錄中有檔案 `order20251206.txt`，資料庫中有前綴為 `order` 的主檔規格（big5 編碼、固定長度格式），**當** 批次服務掃描 NAS 並查詢到對應規格後讀取該檔案並進行轉換，**則** 成功依據欄位長度定義解析資料，並產生正確的 parquet 檔案
4. **假設** NAS 目錄中有檔案使用固定長度格式且資料值包含前後空白，**當** 批次服務根據主檔規格讀取該檔案並進行轉換，**則** 系統自動去除每個欄位的前後空白，產生整潔的 parquet 檔案

---

### 使用者故事 2 - 多檔案批次處理與並行控制 (優先級：P1)

當 NAS 目錄中有多個檔案需要處理且系統部署多個 pod 時，系統管理者需要確保每個檔案只被處理一次，避免重複轉換造成資源浪費或資料不一致。

**為什麼是這個優先級**：在 Kubernetes 多 pod 環境下，並行控制是必須的核心功能。沒有這個機制，系統將無法正確運作，會產生嚴重的資料處理問題。

**獨立測試**：可以透過在資料庫中建立主檔規格，在 NAS 測試目錄中放置多個對應前綴的檔案，模擬多個 pod 同時執行的情境，驗證每個檔案是否只被一個 pod 處理，無需等待後續的遮罩服務完成。

**驗收情境**：

1. **假設** NAS 目錄中有 10 個檔案，且有 4 個 pod 同時運行，**當** 批次服務啟動並掃描 NAS 目錄獲取檔案清單，**則** 每個檔案都能被成功鎖定並由單一 pod 處理，不會出現重複處理
2. **假設** Pod A 正在處理檔案 X 並持有資料庫鎖定，**當** Pod B 掃描到相同的檔案 X 並嘗試處理，**則** Pod B 無法取得該檔案的鎖定，會跳過該檔案並處理下一個可用的檔案
3. **假設** Pod A 在處理檔案時發生異常中斷（pod 重啟或當機），**當** 鎖定逾時後，**則** 其他 pod 能夠重新取得該檔案的鎖定並繼續處理
4. **假設** NAS 目錄中有 5 個檔案，且只有 1 個 pod 運行，**當** 批次服務逐一掃描並處理檔案，**則** 所有檔案都能按順序正確處理完成
...
- **FR-001**：系統必須掃描 NAS 指定目錄中的所有檔案，並透過檔案前綴名稱從資料庫查詢對應的主檔規格（包含編碼類型、資料格式、欄位定義、轉碼型態等資訊）
- **FR-001-1**：資料庫中的主檔規格必須在服務啟動前預先建立（透過 SQL 腳本匯入），不是在服務運行期間動態新增
- **FR-001-2**：系統必須支援檔案前綴比對邏輯，例如檔案 `customer20251206.txt` 應比對到前綴為 `customer` 的主檔規格
- **FR-001-3**：若 NAS 目錄中的檔案無法比對到任何主檔規格，系統必須記錄警告訊息並跳過該檔案
...
- **FR-013**：系統必須透過檔案前綴與主檔規格的前綴欄位進行比對，確定該檔案應使用哪個主檔規格進行解析
- **FR-014**：系統必須在無法比對到主檔規格時，記錄詳細的警告訊息（包含檔案名稱、掃描時間），並跳過該檔案繼續處理其他檔案
...
- **主檔規格（File Specification）**：代表一類檔案的解析規格配置，包含檔案前綴、編碼類型（big5/utf-8）、資料格式類型（分隔符號/固定長度）、分隔符號內容、欄位定義清單等屬性。主檔規格在服務啟動前預先建立於資料庫中，用於解析符合前綴的檔案
- **欄位定義（Field Definition）**：代表主檔規格中每個欄位的詳細定義，包含欄位名稱、欄位順序、資料類型（String/int/double/timestamp）、欄位長度（固定長度格式時使用）、轉碼型態標記等屬性
- **檔案處理任務（File Task）**：代表對單一檔案的處理任務記錄，包含任務 ID、檔案名稱、關聯的主檔規格、處理狀態（待處理、處理中、已完成、處理失敗）、處理開始時間、完成時間、錯誤訊息、前次失敗任務 ID 等屬性
- **Advisory Lock 識別碼**：代表用於並行控制的鎖定識別碼，基於檔案名稱生成，用於確保單一檔案只被一個 pod 處理

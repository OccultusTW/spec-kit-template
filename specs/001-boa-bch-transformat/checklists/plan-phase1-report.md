...
### ⚠️ 錯誤處理與邊界條件 (CHK031-037)

| 檢查項 | 狀態 | 檢查結果 |
|--------|------|---------|
| CHK031 | ✅ | 大檔案記憶體管理：串流處理 + 30,000 batch size |
| CHK032 | ✅ | 並行控制：advisory lock + file_tasks 狀態管理 |
| CHK033 | ⚠️ | SFTP 檔案讀取失敗：research.md 有提及，但 plan.md 未明確列出 |
| CHK034 | ⚠️ | 編碼偵測失敗：research.md 有處理策略，但未在 plan.md 總結 |
| CHK035 | ✅ | 下游服務失敗：tenacity 重試 + 指數退避 |
| CHK036 | ⚠️ | 資料庫連線失敗：連線池有重試機制，但未明確記錄策略 |
| CHK037 | ⚠️ | Parquet 寫入失敗：未在 plan.md 明確列出處理策略 |

**發現問題**：
- ⚠️ CHK033-037：部分錯誤處理策略存在於 research.md，但未在 plan.md 總結
  - **建議**：在 plan.md「技術背景 - 需要釐清的項目」後增加「錯誤處理策略摘要」
  - **影響**：P1 - 實作時可能遺漏錯誤處理場景
...
2. **錯誤處理策略未在 plan.md 總結 (CHK033-037)**
   - **問題**：部分錯誤處理策略散落在 research.md，plan.md 未統整
   - **建議**：在 plan.md 增加「錯誤處理策略摘要」章節
   - **應包含**：
     - SFTP 連線失敗重試機制
     - 編碼偵測失敗降級策略
     - 資料庫連線失敗處理
